...
TEST(Conto,Test_controllo_aggiunta_transazione_vector){
    ...
    Transazioni T1(OperazioniFinanziarie::Versamento,1500,3005,"Andrea",12,12,2004);
...
TEST(Conto,Test_contorllo_rimovo_transazione){
    Conto C ("Andrea","Barella","BancoPosta",121212);
    Transazioni T1(OperazioniFinanziarie::Versamento,1500,3005,"Andrea",12,12,2004);
    Transazioni T2(OperazioniFinanziarie::Versamento,2500,3005,"Roberto",12,12,2004);
    C.aggiungiTransazione(T1);
    C.aggiungiTransazione(T2);
    C.removeTransazione(T1);

    EXPECT_EQ(C.conta_nr_transazioni(),1);
    EXPECT_EQ(C.getSaldo(),2500);

}
TEST(Conto,Testo_controllo_modifica_transazione){
    Conto C ("Andrea","Barella","BancoPosta",121212);
    Transazioni T(OperazioniFinanziarie::Versamento,1500,3005,"Andrea",12,12,2004);
    C.aggiungiTransazione(T);
    C.modificaTransazione(T,123445,300,"Roberta",OperazioniFinanziarie::BonificoEntrata);
    EXPECT_EQ(T.getNumeroConto(),123445);
    EXPECT_EQ(T.getImporto(),300);
    EXPECT_EQ(T.getDestinatario(),"Roberta");
    EXPECT_EQ(T.getTipoOperazione(),OperazioniFinanziarie::BonificoEntrata);
    EXPECT_EQ(C.getSaldo(),300);
